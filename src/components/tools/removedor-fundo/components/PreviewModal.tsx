                        ...
                        <Accordion title={t('previewModal.liveAdjustments')} defaultOpen={true}>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Button variant="secondary" onClick={() => {
                              ...
                              else if (activeTool==='refine') { updateLocalOption('grabcut', { ...(localOptions.grabcut||{}), useWorker: true } as any); generatePreview({ ...localOptions, grabcut: { ...(localOptions.grabcut||{}), useWorker: true } }, localRemovalInfo); }
                              ...
                              <div className="grid grid-cols-2 gap-2 mt-2">
                                <select
                                  ...
                                  onChange={(e) => {
                                    ...
                                    else if (v==='portrait' || v==='product') { updateLocalOption('mode','grabcut' as any); updateLocalOption('grabcut', { ...(localOptions.grabcut||{}), lambda: 50 } as any); }
                             ...
                             <SliderControl id="preview-decontamination" label={t('previewModal.colorDecontamination')} value={localOptions.colorDecontamination ?? 0} min={0} max={100} onChange={(v) => updateLocalOption('colorDecontamination', v)} icon={<DropletIcon className="w-4 h-4 mr-2" />} description={t('previewModal.colorDecontaminationDesc')} />
                              ...
                              {localOptions.mode === 'grabcut' && (
                                <div className="space-y-3 pt-2">
                                  <SliderControl id="grabcut-iters" label={t('previewModal.iterations') || 'Iterações'} value={localOptions.grabcut?.iterations ?? 5} min={1} max={10} onChange={(v) => updateLocalOption('grabcut', { ...(localOptions.grabcut||{}), iterations: v } as any)} />
                                  <SliderControl id="grabcut-lambda" label={t('previewModal.lambda') || 'Suavidade (λ)'} value={localOptions.grabcut?.lambda ?? 50} min={5} max={150} step={5} onChange={(v) => updateLocalOption('grabcut', { ...(localOptions.grabcut||{}), lambda: v } as any)} description={t('previewModal.lambdaDesc') || 'Maior λ mantêm bordas mais suaves; menor, mais detalhadas.'} />
                                  ...
                                  <Button variant="primary" onClick={() => { if (isLoading) return; updateLocalOption('grabcut', { ...(localOptions.grabcut||{}), useWorker: true } as any); setScribbleMode(null); setIsPicking(false); generatePreview({ ...localOptions, grabcut: { ...(localOptions.grabcut||{}), useWorker: true } }, localRemovalInfo); }} disabled={isLoading}>
