...
export const DownloadSelectedButton: React.FC<DownloadSelectedButtonProps> = ({
    ...
    return (
        <div className="relative inline-flex rounded-lg shadow-sm" ref={wrapperRef}>
            ...
            >
                ...
                <span>{feedbackActive ? t('action_bar.starting') : t('action_bar.download_individual')}</span>
                {selectedCount > 0 && !feedbackActive && (
                    <span className="ml-2 bg-indigo-100 text-indigo-700 dark:bg-indigo-500/30 dark:text-indigo-300 text-xs font-bold px-2 py-0.5 rounded-full">{selectedCount}</span>
                )}
            </button>
                ...
                {isOpen && (
                    <div className="absolute right-0 z-10 bottom-full mb-2 w-56 rounded-md shadow-lg bg-light-card dark:bg-dark-card ring-1 ring-black dark:ring-border-gray ring-opacity-5 focus:outline-none">
                        <div className="py-1" role="menu" aria-orientation="vertical">
                            <button
                                onClick={() => { onDownloadAsZip(); setIsOpen(false); }}
                            ...
                            >
                                ...
                                <span>{t('action_bar.download_zip')}</span>
                            </button>
                        </div>
                    </div>
                )}
