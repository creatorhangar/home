...
export function ResultsGallery({ images, productInfo, labelConfig, onEdit, onBack }: ResultsGalleryProps) {
    ...
    const handleDownload = async (preview: GeneratedPreview) => {
        try {
            ...
            if (error.message === 'LIMIT_REACHED') {
                alert("Você atingiu o limite de 5 downloads gratuitos hoje! Faça upgrade para continuar.");
    ...
    const handleBatchDownload = async () => {
        ...
        try {
            await incrementUsage(); // Count as 1 usage? Or N? Let's count as 1 usage per Batch for user friendliness, or selectedIds.size.
            // Let's count as 1 for now to match interface simple logic, or loop?
            // Since incrementUsage throws, calling it once validates "Can I perform an action?".
            // If we want strict counting, we might need a `checkLimit(amount)` method.
            // For MVP: 1 batch = 1 credit used. Fair compromise for UX.
            ...
            if (error.message === 'LIMIT_REACHED') {
                alert("Você atingiu o limite de downloads gratuitos hoje! Faça upgrade para continuar.");
                router.push('/dashboard');
            }
        } finally {
